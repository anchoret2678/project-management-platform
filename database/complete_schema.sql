...
-- 创建数据库
CREATE DATABASE IF NOT EXISTS project_management_platform 
...
CREATE TABLE slas (
    ...
    INDEX idx_title (title),
    INDEX idx_status (status),
    INDEX idx_availability_rate (availability_rate),
    INDEX idx_effective_date (effective_date),
    INDEX idx_created_at (created_at)
...
CREATE TABLE cloud_resources (
    ...
    resource_type VARCHAR(100) NOT NULL COMMENT '资源类型：云服务器/云存储/云数据库/弹性IP',
    resource_quota DECIMAL(10, 2) NOT NULL COMMENT '资源配额',
    used_quota DECIMAL(10, 2) NOT NULL DEFAULT 0 COMMENT '已使用配额',
    provider VARCHAR(100) NOT NULL COMMENT '云服务商：阿里云/腾讯云/华为云/AWS',
    region VARCHAR(50) NOT NULL COMMENT '地域：华东1/华北2/华南3',
    status ENUM('available', 'used', 'reserved', 'expired') DEFAULT 'available' COMMENT '状态',
    effective_date DATE COMMENT '生效日期',
    ...
    INDEX idx_status (status),
    INDEX idx_effective_date (effective_date)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='云资源管理表';
...
('技术文档', '技术架构、API文档、开发规范、代码说明等', 1),
('运维手册', '部署指南、监控配置、故障处理、性能优化等', 2),
('架构文档', '系统架构设计、技术选型、组件关系图等', 3),
('需求文档', '业务需求、功能规格、用户故事、验收标准等', 4),
('会议纪要', '项目会议记录、决策记录、行动计划等', 5);

-- 初始化管理员用户（密码：admin123）
-- 注意：实际部署时应使用bcrypt生成密码哈希
...
('user2', '$2b$12$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'user2@company.com', '李四', 'user', '运维部', '13800138002', TRUE);
