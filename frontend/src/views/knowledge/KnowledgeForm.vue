  ...
  <div class="knowledge-form-container">
    ...
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-title">{{ isEditMode ? '编辑知识文档' : '创建知识文档' }}</h1>
        <p class="page-subtitle">{{ isEditMode ? '修改知识文档信息' : '添加新的知识文档' }}</p>
      </div>
                ...
                >
                  ...
                  <template #tip>
                    <div class="el-upload__tip">
                      支持 PDF、Word、Excel、PPT、TXT 格式，文件大小不超过 100MB
                    </div>
                  </template>
                </el-upload>
...
// 上传配置
const uploadUrl = ref('/api/knowledge/upload') // 实际项目中替换为真实API地址
...
const beforeUpload = (file: UploadRawFile) => {
  ...
  if (!isAllowedType) {
    ElMessage.error('只支持 PDF、Word、Excel、PPT、TXT 格式的文件')
    return false
  }
  ...
  uploadData.value.document_title = formData.document_title
  uploadData.value.category = formData.category
...
const loadData = async () => {
  ...
  try {
    ...
    Object.assign(formData, {
      ...
      document_title: mockData.document_title,
...
onMounted(() => {
  // 设置页面标题
  appStore.setPageTitle(isEditMode.value ? '编辑知识文档' : '创建知识文档')
